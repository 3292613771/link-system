            ...
            for line in f:
                ...
                if "----" in line:
                    ...
                    if len(parts) == 2:
                        ...
                        if '@' not in email: email = email + "@qq.com"
    ...
    try:
        ...
        for part, charset in decode_header(s):
            if isinstance(part, bytes):
                if charset: decoded_parts.append(part.decode(charset, errors='replace'))
                else: decoded_parts.append(part.decode('utf-8', errors='replace'))
            else: decoded_parts.append(str(part))
        ...
        if msg.is_multipart():
            for part in msg.walk():
                ...
                if payload:
                    ...
                    if text.strip(): all_parts.append((part.get_content_type(), text))
        else:
            ...
            if payload:
                ...
                if text.strip(): all_parts.append((msg.get_content_type(), text))
        ...
        if not content:
            for content_type, text in all_parts:
                if content_type == "text/html":
                    ...
                    break
        if not content: return "无法解析邮件内容"
        code = None
        match = re.search(r'(\d)\s*(\d)\s*(\d)\s*(\d)\s*(\d)\s*(\d)', content)
        if match: code = match.group(1)+match.group(2)+match.group(3)+match.group(4)+match.group(5)+match.group(6)
        if not code:
            match = re.search(r'\b(\d{6})\b', content)
            if match: code = match.group(1)
        content = content[:1000]
        if code: return f"验证码：{code}\n\n{content}"
        return content
    except Exception as e: return f"解析失败"

def get_latest_mails(email_addr, limit=1):
    if email_addr not in ACCOUNTS: return {'error': f'邮箱 "{email_addr}" 未绑定'}
                            ...
                            if isinstance(part, tuple):
                                ...
                                try:
                                    if date_str:
                                        ...
                                        timestamp = ts
                                except Exception as e: send_time = date_str[:30] if date_str else ""
                                subject = decode_str(msg.get("Subject", "无主题"))
                                sender = decode_str(msg.get("From", "未知发件人"))
                                content = get_mail_content(msg)
                                all_mails.append({
                                    'mail_id': mail_id_str, 'sender': sender, 'subject': subject,
                                    'content': content, 'time': send_time, 'timestamp': timestamp, 'folder': folder
                                })
                                break
                    except Exception as e: continue
            except Exception as e: continue
        all_mails.sort(key=lambda x: x.get('timestamp', 0), reverse=True)
        for m in all_mails: m.pop('timestamp', None)
        return all_mails[:limit]
    except Exception as e: return {'error': f'连接失败：{str(e)}'}
    finally:
        if mail:
            try: mail.close()
            except: pass
...
def backup_data():
    try:
        ...
        if os.path.exists(ACCOUNTS_FILE): files_to_backup.append(ACCOUNTS_FILE)
        ...
        if os.path.exists(latest_backup): shutil.rmtree(latest_backup)
...
def auto_clean_worker():
    while True:
        ...
        try:
            ...
            if count > 0: print(f"✅ 自动清理了 {count} 个过期链接")
        except Exception as e: print(f"自动清理出错: {e}")
...
# ===== 安全写入 used_emails.json（核心防丢逻辑） =====
def save_used_emails(data):
    temp_file = USED_EMAILS_FILE + '.tmp'
    try:
        with open(temp_file, "w", encoding="utf-8") as f:
            json.dump(data, f, ensure_ascii=False, indent=2)
        os.replace(temp_file, USED_EMAILS_FILE)
    except Exception as e:
...
def admin():
    ...
    all_used = []
    for buyer, emails in used_data.get("records", {}).items(): all_used.extend(emails)
    ...
    for link_id, data in links.items():
        try:
            expire_time = datetime.strptime(data['expire_at'], "%Y-%m-%d %H:%M:%S")
            if now > expire_time and data.get('status') == 'active': expired_count += 1
        except: pass
    data_info = f"""
    <div style="background: #e8f5e9; padding: 10px; border-radius: 8px; margin: 10px 0;">
        <strong>💾 数据持久化状态：</strong><span style="color: green;">✅ 已启用</span>
        <br>数据目录：{DATA_DIR}
    ...
    for link_id, data in links.items():
        ...
        try:
            expire_time = datetime.strptime(data['expire_at'], "%Y-%m-%d %H:%M:%S")
            if now > expire_time: status = '⏰ 已过期'
        except: pass
        link_list += f"""
        <tr>
            <td>{link_id}</td><td>{data.get('buyer_id', '未知')}</td><td>{data.get('type', '未知')}</td>
            <td>{len(data['emails'])}</td><td>{data['created_at']}</td><td>{data['expire_at']}</td><td>{status}</td>
            <td><a href="/api/disable_link?link_id={link_id}" onclick="return confirm('确定要禁用此链接吗？')" style="color: red; text-decoration: none;">⛔ 禁用</a></td>
        </tr>
        """
    html_admin = f'''
    <!DOCTYPE html>
    ...
    <body style="font-family: Arial; max-width: 1200px; margin: 20px auto; padding: 20px;">
        ...
        <div style="background: #fff3cd; padding: 15px; border-radius: 8px; margin: 20px 0;">
            ...
            <form action="/api/disable_link" method="get" style="display: flex; gap: 10px; align-items: center;">
                <input type="text" name="link_id" placeholder="输入链接ID" style="padding: 10px; font-size: 14px; border: 2px solid #ddd; border-radius: 8px; flex: 1;">
                <button type="submit" onclick="return confirm('确定要禁用此链接吗？')" style="padding: 10px 20px; background: #dc3545; color: white; border: none; border-radius: 8px; cursor: pointer; font-size: 14px;">确认失效</button>
            </form>
        </div>
        <hr>
        <h3>➕ 手动生成链接</h3>
        <form action="/api/admin_create_link" method="post">
            ...
            <br>
            <select name="type"><option value="数字">数字邮箱</option><option value="英文">英文邮箱</option><option value="foxmail">foxmail邮箱</option></select>
            <input type="number" name="days" value="30" style="width:80px;"><label>天</label>
            <br><br><button type="submit" style="padding:10px 30px;background:#4CAF50;color:white;border:none;cursor:pointer;">生成链接</button>
        </form>
        <hr>
        <h3>📊 库存统计</h3>
        <ul><li>总邮箱数：{total}</li><li>已分配：{used}</li><li>可用：{total - used}</li></ul>
        <div style="background: #e3f2fd; padding: 15px; border-radius: 8px; margin: 20px 0;">
            <h3>💾 数据备份（实时备份已启用）</h3>
            <p>最新备份时间：{get_latest_backup_time()}</p>
            <p style="color: green; font-size: 14px;">✅ 生成/禁用/清理链接时自动备份</p>
            <a href="/api/manual_backup" style="padding: 10px 20px; background: #2196F3; color: white; text-decoration: none; border-radius: 8px; display: inline-block;">立即备份</a>
            <a href="/api/download_latest_backup" style="padding: 10px 20px; background: #4CAF50; color: white; text-decoration: none; border-radius: 8px; display: inline-block; margin-left: 10px;">下载最新备份</a>
        </div>
        <div style="background: #fff3e0; padding: 15px; border-radius: 8px; margin: 20px 0;">
            <h3>🧹 数据清理</h3>
            <p>当前过期链接数：<strong style="color: red;">{expired_count}</strong></p>
            <a href="/api/clean_expired_links" onclick="return confirm('确定要清理所有过期链接吗？此操作不可恢复！')" style="padding: 10px 20px; background: #ff9800; color: white; text-decoration: none; border-radius: 8px; display: inline-block;">🗑️ 清理过期链接（{expired_count}个）</a>
        </div>
        <hr>
...
def get_latest_backup_time():
    ...
    if os.path.exists(latest_backup):
        return datetime.fromtimestamp(os.path.getmtime(latest_backup)).strftime("%Y-%m-%d %H:%M:%S")
    return "暂无备份"
...
def disable_link():
    ...
    if not link_id: return "请提供链接ID", 400
    links = load_links()
    if link_id not in links: return f"链接 {link_id} 不存在", 404
    links[link_id]['status'] = 'disabled'
    save_links(links)
    backup_data()
    return f'<h2>✅ 链接已失效</h2><p>链接ID: {link_id}</p><p>状态已更新为: 禁用</p><p>数据已保存并备份，重新部署不会丢失</p><a href="/admin">返回后台</a>'

@app.route('/api/clean_expired_links')
def api_clean_expired_links():
    if not session.get('logged_in'): return redirect('/login')
    cleaned_count = clean_expired_links()
    return f'<h2>✅ 清理完成</h2><p>清理了 {cleaned_count} 个过期链接</p><p>数据已备份</p><a href="/admin">返回后台</a>'

@app.route('/api/manual_backup')
def manual_backup():
    if not session.get('logged_in'): return redirect('/login')
    success = backup_data()
    if success: return f'<h2>✅ 备份成功</h2><p>数据已备份到: {BACKUP_DIR}</p><p>重新部署后可从该目录恢复数据</p><a href="/admin">返回后台</a>'
    else: return '<h2>❌ 备份失败</h2><p>请检查服务器权限</p><a href="/admin">返回后台</a>'

@app.route('/api/download_latest_backup')
def download_latest_backup():
    if not session.get('logged_in'): return redirect('/login')
    latest_backup = os.path.join(BACKUP_DIR, "latest")
    if not os.path.exists(latest_backup): return "暂无备份文件"
    import zipfile
...
def admin_create_link():
    ...
    if not emails: return "请至少输入一个邮箱", 400
    link_id = str(uuid.uuid4())[:8]
    links = load_links()
    now = datetime.now()
    links[link_id] = {
        'link_id': link_id, 'buyer_id': 'admin', 'type': type_name, 'emails': emails,
        'quantity': len(emails), 'created_at': now.strftime("%Y-%m-%d %H:%M:%S"),
        'expire_at': (now + timedelta(days=days)).strftime("%Y-%m-%d %H:%M:%S"),
        'status': 'active', 'query_count': 0
    }
    save_links(links)
    backup_data()
    link_url = f"https://{DOMAIN}/query?link={link_id}"
    return f'生成成功！<br>链接：<a href="{link_url}" target="_blank">{link_url}</a><br>邮箱：{", ".join(emails)}<br>有效期：{links[link_id]["expire_at"]}<br><a href="/admin">返回后台</a>'
...
def auto_create_link():
    ...
    except (TypeError, ValueError): return "quantity 和 days 必须为整数", 400
    buyer_id = str(data.get('buyer_id') or str(uuid.uuid4())[:8])
    if quantity <= 0: return "数量必须大于0", 400
    valid_types = ['数字', '英文', 'foxmail']
    if type_name not in valid_types: return f"无效类型，请选择: {', '.join(valid_types)}", 400
    selected_emails, error = assign_emails(type_name, quantity, buyer_id)
    if error: return f"分配失败：{error}", 400
    link_id = str(uuid.uuid4())[:8]
    links = load_links()
    now = datetime.now()
    links[link_id] = {
        'link_id': link_id, 'buyer_id': buyer_id, 'type': type_name, 'emails': selected_emails,
        'quantity': quantity, 'created_at': now.strftime("%Y-%m-%d %H:%M:%S"),
        'expire_at': (now + timedelta(days=days)).strftime("%Y-%m-%d %H:%M:%S"),
        'status': 'active', 'query_count': 0
    }
    save_links(links)
...
def query_page():
    ...
    if not link_id: return "缺少链接ID"
    links = load_links()
    if link_id not in links: return "链接不存在"
    link_data = links[link_id]
    now = datetime.now()
    try:
        expire_time = datetime.strptime(link_data['expire_at'], "%Y-%m-%d %H:%M:%S")
        if now > expire_time: return "⛔ 链接已过期"
    except: pass
...
def query_mail():
    ...
    if not email: return "请输入邮箱"
    email = email.strip()
    if '@' not in email: email = email + "@qq.com"
    links = load_links()
    if link_id not in links: return "链接无效"
    link_data = links[link_id]
    if link_data['status'] != 'active': return "⛔ 链接已被禁用"
    ...
    if isinstance(result, dict) and 'error' in result: return f"查询失败：{result['error']}"
    if not result: return "<h3>📭 暂无邮件</h3>"
    mail = result[0]
    folder_label = ""
    if mail.get('folder') and ('垃圾' in str(mail.get('folder')) or 'Spam' in str(mail.get('folder'))):
    ...
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 20px auto; padding: 20px;">
        ...
        <div style="border: 1px solid #ddd; border-radius: 8px; padding: 15px; background: #f9f9f9;">
            <div style="margin-bottom: 10px;"><span style="font-weight: bold; color: #333;">发件人：</span><span>{mail['sender']}</span></div>
            <div style="margin-bottom: 10px;"><span style="font-weight: bold; color: #333;">主题：</span><span>{mail['subject']}</span></div>
            <div style="margin-bottom: 10px;"><span style="font-weight: bold; color: #333;">时间：</span><span style="color: #666;">{mail.get('time', '未知')}</span></div>
            <div style="border-top: 1px solid #eee; padding-top: 10px;">
                <div style="font-weight: bold; color: #333; margin-bottom: 5px;">邮件内容：</div>
                <div style="white-space: pre-wrap; word-break: break-word; color: #555;">{mail['content'][:2000]}</div>
            </div>
        </div>
        <div style="margin-top: 20px; text-align: center;"><a href="/query?link={link_id}" style="color: #4CAF50; text-decoration: none;">返回查询</a></div>
    </div>
    """
    return html_result
...
def api_admin_logs(): return jsonify({"logs": []})
...
def test_login():
    ...
    try:
        ...
        mail.logout()
        return "登录成功"
    except Exception as e: return f"登录失败：{str(e)}"
...
if __name__ == '__main__':
    ...
    if cleaned > 0: print(f"✅ 启动时清理了 {cleaned} 个过期链接")
    backup_data()
